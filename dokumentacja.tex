...
\begin{itemize}
    \item dla drzew ID3: domyślnie \textbf{Information Gain} oraz opcjonalnie \textbf{Gain Ratio},
...
\begin{enumerate}
    ...
    \begin{description}
        \item[Typ zadania:] klasyfikacja binarna (czy klient zalegnie z płatnością w przyszłym miesiącu?)
        \item[Liczba przykładów:] \textbf{TODO}
        \item[Brak zaległości:] \textbf{TODO}
        \item[Zaległość:] \textbf{TODO}
    ...
    \begin{description}
        ...
        \item[Uwagi:] Klasy są niezbalansowane -- trzeba zadbać, aby nie wpłynęło to na algorytm. Atrybuty ciągłe zostaną zdyskretyzowane lub obsłużone progiem (CART). Zbiór powinien dobrze pokazywać różnice przy testowaniu różnych funkcji oceny jakości testu. Nie spodziewamy się dokładności 100\%.
    \end{description}
...
\begin{table}[h!]
\centering
\caption{Domyślne hiperparametry hiperparametryczne dla modeli leśnych}
...
Dla każdego badania przeanalizowane zostały dokładność, czułośc, precyzja, wynik F1 oraz czas trenowania lasu.
W celu utrzymania przejrzystości dokumentu wyniki zostaną zaprezentowane w większości w postaci zaagregowanych wykresów.
...
\begin{itemize}
    ...
    Drzewa CART radzą sobie gorzej od ID3 - 100\% dokładności osiągają dopiero przy 50 drzewach. Wynika to z oddmiennego sposobu
    ...
    Powyżej 50 drzew miary czułości, precyzji i F1-score są delikatnie poprawiane wraz ze wzrostem liczby drzew.
    ...
    wskazują na trudność zbioru danych. Algorytm ma problem z wykryciem klasy pozytywnej i w znacznej ilości przypadków działa podobnie
...
Jego mniejsze wartości zwiększają różnorodnośc drzew, co może prowadzić do redukcji zjawiska przeuczania.
...
\begin{itemize}
    ...
    W przypadku radzeniem sobie ze zwiększoną liczbą atrybutów (one-hot encoding) większy rozmiar turnieju pozwala na
    wybór bardziej reprezentatywnych cech podziału, co przekłada się na lepsze wyniki modelu. Zwiększenie szranek pozwala
...
\subsubsection{Wnioski ogólne}
\begin{itemize}
    \item Wzrost liczebności turnieju zazwyczaj nie wpływa znacząco na dokładnośc modelu.
...
\begin{itemize}
    ...
    byc to uznane za szum statystyczny, jednak z uwagi na brak innych przesłanego zostanie ona wybrana jako domyślna dla drzew ID3 w porównaniu modeli.
...
Przyjęte hiperparametry zostały skorygowane o wyniki z poprzedniego punktu badań. Zastosowane wartości to:
